...
function OrgSettings() {
  ...
  useEffect(() => {
    if (o && !form) {
      setForm({ name: o.name, type: o.type || '', taxId: o.taxId || '', address: o.address || '', phone: o.phone || '', mobile: o.mobile || '', email: o.email || '', website: o.website || '', currency: o.currency || 'THB', language: o.language || 'en' });
    }
  }, [o]);
...
function TeamSettings() {
  ...
  const ROLE_BADGE: Record<string, any> = { OWNER: 'orange', ADMIN: 'red', MANAGER: 'blue', STAFF: 'gray' };
    ...
    <div className="space-y-5">
      ...
      {isLoading ? <LoadingSpinner /> : (
        <div className="space-y-2">
          {list.map((m: any) => (
            <div key={m.id} className="flex items-center justify-between p-3 rounded-xl border border-slate-200 bg-slate-50">
              ...
              <div className="flex items-center gap-3">
                ...
                {m.role !== 'OWNER' && (
                  <button onClick={() => removeMutation.mutate(m.user?.id)} className="p-1.5 text-slate-400 hover:text-red-500 transition-colors">
                    <Trash2 size={14} />
                  </button>
                )}
              </div>
      ...
      <Modal open={showInvite} onClose={() => setShowInvite(false)} title="Invite Team Member">
        <div className="space-y-4">
          ...
          {inviteMutation.isError && <p className="text-sm text-red-500">User not found.</p>}
        </div>
      </Modal>
    </div>
  );
...
function PasswordSettings() {
  ...
  return (
    <div className="space-y-5 max-w-md">
      ...
      {mutation.isSuccess && <div className="bg-green-50 border border-green-200 text-green-700 text-sm px-4 py-3 rounded-lg">✓ Password changed</div>}
